...
bool frame_func() {
    ...
    switch (trans) {
        ...
        case 2:
            for (i = 0; i < nRows; i++)
                for (j = 0; j < nCols; j++) {
                    const auto r = sqrtf(powf(j - static_cast<float>(nCols) / 2, 2)
...
int main() {
    ...
    hge->System_SetState(HGE_LOGFILE, "run.log");
...
// Load sound and texture
        tex = hge->Texture_Load("res/texture.jpg");
        if (!tex) {
